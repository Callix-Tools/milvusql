"""Schema for the `images` collection.

Runs entirely through `milvusql+aio`, the same async engine request
handling (`main.py`) uses: a real Milvus server (this example's
...
from sqlalchemy import BigInteger, Column, Index, MetaData, String, Table, inspect
...
async def bootstrap_schema(engine: AsyncEngine) -> None:
    """Create the collection and index it. Safe to call on every app
    startup: each step is guarded so re-running it against an
