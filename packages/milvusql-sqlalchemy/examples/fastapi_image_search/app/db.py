"""Schema and engines for the `images` collection.

`bootstrap_schema()` runs through a *sync* engine, even though request
handling (`app.main`) is fully async -- `CREATE INDEX` waits for
...
client and only switching to async for request handling sidesteps
...
images = Table(
    ...
    Column("caption", String(512)),
    Column("embedding", VECTOR(VECTOR_DIM)),
    milvusql_shards=1,
    milvusql_consistency_level="Strong",
)

EMBEDDING_INDEX_NAME = "idx_images_embedding"


def bootstrap_schema() -> None:
    """Create the collection, index it, and load it. Safe to call on
    every app startup: each step is guarded so re-running it against
    an already-bootstrapped database is a no-op, not an error. This is
    a demo convenience, not a migration story -- a real deployment
